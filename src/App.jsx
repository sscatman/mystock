...
/**
 * AI Hyper-Analyst GLOBAL V1.04
 * 업데이트 내역:
 * 1. 버전 업데이트: V1.032 -> V1.04
 * 2. 검색 유연성 강화: 티커 뿐만 아니라 주식 이름(한글/영문) 입력 시에도 차트 검색 시도
 * 3. KRX 심볼 매핑 개선: 숫자 6자리 입력 시 최우선 순위 부여
 * 4. UI 안내 문구 수정: 티커와 이름 모두 사용 가능하도록 가이드 업데이트
 */
...
const getTradingViewSymbol = (input) => {
  ...
  const isKORNumber = /^\d{6}$/.test(cleanInput); // 6자리 숫자면 한국 주식 코드
  
  if (isKORNumber) {
    return `KRX:${cleanInput}`;
  }
  
  // 이름이나 영문 티커인 경우 그대로 전달하여 트레이딩뷰 자체 검색 엔진 활용
  return cleanInput.toUpperCase(); 
};

const fetchRealStockData = async (ticker) => {
  // 숫자형 티커가 아닌 이름 형태면 공공데이터 API 조회가 어려우므로 null 반환
...
const TradingViewWidget = ({ symbol }) => {
  ...
  useEffect(() => {
    ...
    const renderWidget = () => {
      // 심볼에서 특수문자를 제거한 고유 ID 생성 (한글 포함 시 대비하여 인코딩 처리)
      ...
      if (container.current && window.TradingView) {
        ...
        new window.TradingView.widget({
          ...
          "allow_symbol_change": true, // 사용자가 차트 내부에서 종목을 바꿀 수 있게 허용
          "container_id": safeId,
          "studies": [
            "RSI@tv-basicstudies",
    ...
    <div className="flex flex-col lg:flex-row h-screen bg-[#0f172a] font-sans text-slate-200 overflow-hidden">
      ...
      <div className="lg:hidden flex items-center justify-between p-4 bg-slate-900 border-b border-slate-800 z-30">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-extrabold text-xs uppercase tracking-tighter">Hyper Analyst Global V1.04</span>
        </div>
      ...
      `}>
        ...
        <div className="flex p-6 border-b border-slate-700 items-center space-x-3 bg-[#1e293b]">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-slate-400 font-mono tracking-widest uppercase font-bold text-left">V1.04 FLEXIBLE</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
          ...
          <div className="space-y-2 pt-2">
            ...
            <p className="text-[10px] text-slate-500 px-1 italic text-left">예: 삼성전자, 005930, NVDA, Apple</p>
          </div>
        </div>
          ...
          <div className="max-w-6xl mx-auto w-full pb-20 space-y-10">
              ...
              <div className="flex items-start space-x-5">
                ...
                <div>
                  <div className="flex items-center">
                    ...
                    <span className="text-sm font-normal text-slate-500 ml-4 not-italic font-mono">V1.04</span>
                  </div>
                  <p className="text-slate-500 text-sm mt-3 font-medium uppercase tracking-widest text-left">Professional Market Intelligence Engine</p>
                </div>
              </div>
            </div>
            ...
            {activeSymbol && (
              <div className="animate-in fade-in zoom-in-95 duration-500">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <h2 className="text-sm font-black uppercase tracking-widest text-slate-300">Live Market Intelligence Chart ({activeSymbol})</h2>
                </div>
                <TradingViewWidget symbol={activeSymbol} />
                <p className="mt-3 text-[11px] text-slate-500 italic">
                  * 차트가 나오지 않는다면 해당 검색어로 종목을 찾지 못한 것입니다. 정확한 티커(예: 005930) 입력을 권장합니다.
                </p>
              </div>
            )}
            ...
            {!generatedPrompt && !isGenerating && (
              <div className="mt-16 flex flex-col items-center justify-center text-center space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-1000">
                ...
                <h3 className="text-xl lg:text-2xl font-black text-white uppercase tracking-tighter text-left">Ready for Global Analysis</h3>
                <p className="text-slate-500 text-sm max-w-md mx-auto leading-relaxed text-left">
                  {activeSymbol ? '차트 분석을 마쳤다면 분석 지침서를 생성하세요.' : '왼쪽 사이드바에서 기업명이나 티커를 입력하고 분석을 시작하세요.'}
                </p>
              </div>
            )}
            ...
            {isGenerating && (
              <div className="mt-12 flex flex-col items-center space-y-6">
                ...
                <p className="text-slate-500 text-sm font-medium animate-pulse text-left">시장 데이터를 동기화하고 정밀 분석 프롬프트를 구성 중입니다...</p>
              </div>
            )}
            ...
            {generatedPrompt && (
              <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8">
                <div className="bg-indigo-600/10 border border-indigo-500/20 rounded-3xl p-6 flex items-start space-x-5 shadow-inner">
                  ...
                  <div>
                    <h4 className="text-white font-bold text-sm uppercase tracking-wide text-left">Analysis Intelligence Prompt Ready</h4>
                    <p className="text-indigo-100/60 text-xs lg:text-sm leading-relaxed mt-1 text-left">모든 분석 지침과 데이터 서식이 포함되었습니다. 제미나이에 붙여넣어 결과를 확인하세요.</p>
                  </div>
                </div>
                ...
                <div className="relative group">
                  ...
                  <div className="relative bg-[#0d1326] border border-slate-700/50 rounded-[2rem] overflow-hidden shadow-2xl">
                    <div className="flex items-center justify-between px-8 py-5 border-b border-slate-700/50 bg-slate-800/30">
                      <span className="text-[11px] text-slate-400 font-mono font-black uppercase tracking-[0.2em]">Wall-Street Grade Full Output (V1.04)</span>
