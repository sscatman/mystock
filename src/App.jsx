...
/**
 * AI Hyper-Analyst GLOBAL V0.8
 * 업데이트 내역:
 * 1. 버전 표시 위치 수정: 메인 타이틀 옆에 "V0.8" 고정
 * 2. 프롬프트 엔진 보강: 가치평가 결과 표 및 DCF 계산 가정 표 형식 풀텍스트 포함
 * 3. 가치평가 평균(적정 주가 밴드) 산출 지침 추가
 */
...
[재무 지표]
N/A (최신 분기 리포트 및 지표를 직접 수집하여 분석에 반영하십시오.)

[관련 뉴스]
${useNews ? `- [Google News] (관련 최신 뉴스를 리스트화 하십시오.)` : ""}
...
4. **체크리스트 확인**: 분석 완료 후, 모든 항목(${analysisItems.join(', ')})이 포함되었는지 **스스로 검증**하십시오. 누락 시 즉시 추가 작성하십시오.

---

0. **[기업 기본 정보 (Company Overview)]**
   - 보고서 서두에 다음 데이터를 사용하여 **마크다운 표**를 작성하십시오.
   | 항목 | 내용 |
   |---|---|
...
1. **[성장주/가치주 정의 및 핵심 지표 분석]**
   - 이 기업이 '성장주'인지 '가치주'인지 규명하고 근거를 제시하십시오.
...
4. **[시나리오별 확률 및 근거 (Scenario Analysis)]**
   - Bull, Base, Bear 3가지 시나리오와 확률(%)을 제시하십시오.

[결론]
반드시 [매수 / 매도 / 관망] 중 하나의 명확한 투자 의견을 제시하십시오.

⚠️ **[최종 검증]** 모든 항목이 분석되었는지 확인 후 답변을 마칩니다.
    `.trim();
    ...
    <div className="flex flex-col lg:flex-row h-screen bg-[#0f172a] font-sans text-slate-200 overflow-hidden">
      ...
      <div className="lg:hidden flex items-center justify-between p-4 bg-slate-900 border-b border-slate-800 z-30">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-extrabold text-xs uppercase tracking-tighter">Hyper Analyst Global V0.8</span>
        </div>
      ...
      `}>
        <div className="hidden lg:flex p-6 border-b border-slate-700 items-center space-x-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-slate-400 font-mono tracking-widest uppercase font-bold">Terminal Mode</span>
          </div>
        </div>
        ...
        <div className="p-6 flex-1 flex flex-col space-y-5 overflow-y-auto custom-scrollbar pt-24 lg:pt-6">
          ...
          <div className="space-y-2 pt-2">
            <div className="relative group">
              ...
              <input type="text" value={ticker} onChange={(e) => setTicker(e.target.value)} placeholder="AAPL, NVDA, 005930..." className="w-full pl-10 pr-4 py-3.5 bg-slate-900 border border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-rose-500 outline-none transition-all" onKeyDown={(e) => e.key === 'Enter' && handleGeneratePrompt()} />
            </div>
          </div>
        </div>
          ...
          <div className="max-w-5xl mx-auto w-full pb-20">
              ...
              <div className="flex items-start space-x-5">
                ...
                <div>
                  <h1 className="text-3xl lg:text-4xl font-black tracking-tighter text-white italic uppercase leading-none">
                    ...
                    <span className="text-sm font-normal text-slate-500 ml-4 not-italic">V0.8</span>
                  </h1>
                  <p className="text-slate-500 text-sm mt-3 font-medium uppercase tracking-widest">Advanced Professional Prompt Engine</p>
                </div>
              </div>
            </div>
            ...
            {!generatedPrompt && !isGenerating && (
              <div className="mt-16 lg:mt-24 flex flex-col items-center justify-center text-center space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-1000">
                ...
                <h3 className="text-xl lg:text-2xl font-black text-white uppercase tracking-tighter">Ready to Build Prompt</h3>
                <p className="text-slate-500 text-sm max-w-md mx-auto leading-relaxed">종목을 입력하고 하단의 버튼을 눌러주세요. 가치평가 테이블과 심층 분석 지침이 포함된 질문지를 생성합니다.</p>
              </div>
            )}
            ...
            {isGenerating && (
              <div className="mt-24 flex flex-col items-center space-y-6">
                ...
                <p className="text-slate-500 text-sm font-medium animate-pulse">금융 데이터 동기화 및 정밀 지침 구성 중...</p>
              </div>
            )}
            ...
            {generatedPrompt && (
              <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8">
                <div className="bg-indigo-600/10 border border-indigo-500/20 rounded-3xl p-6 flex items-start space-x-5">
                  ...
                  <div>
                    <h4 className="text-white font-bold text-sm uppercase tracking-wide">분석 지침서 생성 완료 (V0.8)</h4>
                    <p className="text-indigo-100/60 text-xs lg:text-sm leading-relaxed mt-1">가치평가(Graham/DCF) 테이블 형식이 포함되었습니다. 복사 후 제미나이(Gemini)에 붙여넣으세요.</p>
                  </div>
                </div>
